...
public sealed class SemanticAnalyzer : IAstVisitor<object?> {
	...
	private int _pass;
	private bool _autoGenerateRoutineLabels;
	...
	/// </summary>
	public bool AutoGenerateRoutineLabels {
		get => _autoGenerateRoutineLabels;
		set => _autoGenerateRoutineLabels = value;
	}
	...
	public object? VisitInstruction(InstructionNode node) {
		...
		if (_autoGenerateRoutineLabels && _pass == 1) {
	...
	private void TryAutoGenerateLabel(InstructionNode node) {
		...
		if (_symbolTable.Symbols.ContainsKey(labelName)) {
			return;
		}

		// Define the auto-generated label
		_symbolTable.Define(labelName, SymbolType.Label, targetAddress, node.Location);
	...
	private bool HasLabelAtAddress(long address) {
		return _symbolTable.Symbols.Values.Any(s =>
